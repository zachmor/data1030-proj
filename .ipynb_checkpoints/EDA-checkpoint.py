...
# quiver_recent_metadata()

def msft_insider_trading():

    # Decided only to work with MSFT, downloaded into data/quiver
...
# msft_insider_trading()

def msft_candlesticks():
    
    alpha = pd.read_csv("data/alpha/MSFT.csv", index_col='time', parse_dates=['time'])[:150]
...
def inefficiencies(df, window_period, threshold_rate, confirmation_window=None, round_up_week=False):
    ...
    ineffy[underpriced.astype(bool)] = 0
    ineffy[overpriced.astype(bool)] = 1
    ineffy[efficient.astype(bool)] = 2
    ineffy[volatile.astype(bool)] = 3
